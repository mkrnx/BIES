...
import { Users, Briefcase, Loader2, UserMinus } from 'lucide-react';
...
const InvestorFollowing = () => {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            setLoading(true);
            try {
                const res = await profilesApi.getFollowing(user.id, { limit: 50 });
    ...
    return (
        <>
            <div className="flw-header">
                <h1>Following</h1>
                <p className="flw-subtitle">Builders and investors you're keeping track of</p>
            </div>

            <div className="flw-tabs">
                {TABS.map(tab => {
                    const Icon = tab.icon;
                    const count = tab.key === 'builders' ? builders.length : investors.length;
                    return (
                        <button
                            key={tab.key}
            ...
            <style>{`
                ...
                .flw-tab.active { color: var(--color-secondary, #F97316); border-bottom-color: var(--color-secondary, #F97316); font-weight: 600; }
                .flw-tab-count {
                ...
                .flw-tab.active .flw-tab-count { background: #FFF7ED; color: var(--color-secondary, #F97316); }
                ...
                .flw-card:hover { border-color: #fdba74; box-shadow: 0 2px 12px rgba(249,115,22,0.06); }
                .flw-card-top { display: flex; align-items: center; gap: 0.75rem; }

                .flw-avatar {
                    width: 44px; height: 44px; border-radius: 50%;
                    background: linear-gradient(135deg, var(--color-secondary, #F97316), #F59E0B);
                ...
                .flw-name:hover { color: var(--color-secondary, #F97316); }
                ...
                .flw-btn-link {
                    ...
                    background: var(--color-secondary, #F97316); color: white;
...
export default InvestorFollowing;
